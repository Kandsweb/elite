...
// show case only superceeds all other settings
    if (STORE_STATUS != '0') {
      return '<a href="' . zen_href_link(FILENAME_CONTACT_US, 'pId='.$product_id) . '">' .  zen_image_button('button_contact_us.gif','Enquire about this item') . '</a>';
    }
    ...
    // verify display of prices
      switch (true) {
        ...
        if (TEXT_LOGIN_FOR_PRICE_PRICE == '') {
          ...
          //1customer may browse but no prices
          //KandS comment out next line
          //$login_for_price = '<a href="' . zen_href_link(FILENAME_LOGIN, '', 'SSL') . '">' .  TEXT_LOGIN_FOR_PRICE_BUTTON_REPLACE . '</a>';
          $login_for_price='<br /><a href="' . zen_href_link(FILENAME_CONTACT_US, 'pId='.$product_id) . '">' .  zen_image_button('button_contact_us.gif','Enquire about this item') . '</a>';
        }
        return $login_for_price;
        break;
        // show room only
