...
class SimpleCategoriesTree{
  ...
  var $nolinkchild = '';
	...
	function buildCategoryString($parent_html = '<ul class="{class}">{child}</ul>', $child_html = '<li class="{class}"><a class="{class} category-top" href="{link}">{name}</a>{child}</li>',
				$categories_id = 0, $default_current_id = 1, $max_level = 9, $include_root = false, $strict = false){
  ...
  function __buildCategoryStringS($categories_id, $level, $max_level, $strict){
    $result = '';
    if(($level < $max_level) || (!$strict && in_array($categories_id, $this->exceptional_list))){
      //$class = $this->_buildClass($categories_id, $level);
      $parent_level = $level;

       if($this->countSubCategories($categories_id) > 0){
        //$child_html = str_replace(array('{class}','{name}','{child}'), array("{class$level}","{name$level}","{child$level}"),$this->nolinkchild);
       // if(isset($this->category_tree[$categories_id]['sub_cats'])){
          $result = '<div id="nolinkchild">' . $this->category_tree[$categories_id]['name'] . '</div>';
        } else {
          $result = '<div id="linkchild"><a href="' . $this->_buildLink($categories_id) . '">' . $this->category_tree[$categories_id]['name'] . '</a></div>';
       // }
      }
      ...
      if(isset($this->category_tree[$categories_id]['sub_cats'])){
          ...
          for($i=0; $i < $count; $i++){
            //$child_class = $this->_buildClass($this->category_tree[$categories_id]['sub_cats'][$i], $level);
            $child .= $this->__buildCategoryStringS($this->category_tree[$categories_id]['sub_cats'][$i],$level,$max_level,$strict);
          }
          if($level < $max_level){
            //$class = $this->_buildClassS($categories_id, $level);
            //$child = str_replace(array("{class}","{child}"), array($class, $child), $this->parent_html);
            //$result = str_replace("{child$parent_level}", $child, $result);
            $result .= $child;
          }
  ...
  function buildCategoryStringSa($categories_id = 0, $default_current_id = 1, $max_level = 9, $include_root = false, $strict = false){
    $result = '';
    $result .= '<div class="level_Stop"><a href="' . HTTP_SERVER . DIR_WS_CATALOG . '">' . $this->category_tree[$categories_id]['name'] . '</a></div>';

    if(isset($this->exceptional_list[1])){
      $categories_id = $this->exceptional_list[1];
    }

    //$result .= '<li class="level_S1 has_sub currentParent"><span>' . $this->category_tree[$categories_id]['name'] . '</span></li>';
    $result .= '<div class="level_Stop1">' . $this->category_tree[$categories_id]['name'] . '</div>';
    ...
    if(isset($this->category_tree[$categories_id]['sub_cats'])){
        ...
        $count = count($this->category_tree[$categories_id]['sub_cats']);
        for($i=0; $i < $count; $i++){
          $result .= $this->__buildCategoryStringS($this->category_tree[$categories_id]['sub_cats'][$i],$level,$max_level,$strict);
        }
    }
		    //Static links
    $result .= '<div class="level_divider">&nbsp;</div>';
    $result .= '<div class="level_Rlink"> <a href="' . $this->_buildLink($subCat) . '">' . 'Size Chart' . '</a></div>';

    return $result;
  }

  function _buildCategoryStringRoot($categories_id = 0, $level, $default_current_id = 1, $max_level = 9, $include_root = false, $strict = false){
    ...
    if(isset($this->category_tree[$categories_id]['sub_cats'])){
      foreach($this->category_tree[$categories_id]['sub_cats'] as $key => $subCat){
        //Output level as linkable
        $child  = '<div class="linkRootchild"><a href="' . $this->_buildLink($subCat) . '">' . $this->category_tree[$subCat]['name'] . '</a></div>';
        $result .= $child;
        //now process any sub cats this cat has
         //check level
        if ($level < $max_level){
          $result .= $this->_buildCategoryStringRoot($subCat,$level,$default_current_id,$max_level,true,true);
        }else{
          if (isset($this->category_tree[$subCat]['sub_cats'])){
					  $result .=  $zen_CategoriesUL->buildBranch($subCat,1);
          }
				}
      }//eof foreach
    $level--;
  ...
  function buildCategoryStringRoot($categories_id = 0, $default_current_id = 1, $max_level = 9, $include_root = false, $strict = false){
    require_once (DIR_WS_CLASSES . 'categories_ul_generator.php');
    $zen_CategoriesUL = new zen_categories_ul_generator;
    ...
    $result = '<div class="category_tree">';
    $level = 1;
    if(isset($this->category_tree[$categories_id]['sub_cats'])){
      foreach($this->category_tree[$categories_id]['sub_cats'] as $key => $subCat){
        //Output first level as non linkable
        $result .= '<div class="level_Rtop"><a href="' . HTTP_SERVER . DIR_WS_CATALOG . '">' . $this->category_tree[$subCat]['name'] . '</a></div>';
        //now process any sub cats this cat has
    ...
    //Static links
    $result .= '<div class="level_divider">&nbsp;</div>';
    //$result .= '<div class="level_Rlink"> <a href="' . HTTP_SERVER . DIR_WS_CATALOG . '?main_page=page_2' . '">' . 'Size Chart' . '</a></div>';
