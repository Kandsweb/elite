...
if (isset($_POST['action']) && ($_POST['action'] == 'process')) {
  ...
  //BOE KandS - Company fields
  $acc_type = zen_db_prepare_input($_POST['userType']);
  $company = zen_db_prepare_input($_POST['company']);
  $vat_num = zen_db_prepare_input($_POST['vatNum']);
  $trading_type = zen_db_prepare_input($_POST['trading-type']);
  $trade_type = zen_db_prepare_input($_POST['trade-type']);
  $mobile = zen_db_prepare_input($_POST['mobile']);

  if($trade_type==99)$trade_type=zen_db_prepare_input($_POST['trade-other']);
  //EOE //////
 ...
 // }
  ...
  if (strlen($postcode) < ENTRY_POSTCODE_MIN_LENGTH) {
    ...
    //BOE KandS - NOPOSTCODE remove
    if($postcode == 'NOPOSTCODE')$postcode='';
    //EOF KandS
  ...
  if ($error == true) {
    ...
    $sql_data_array = array('customers_firstname' => $firstname,
                            ...
                            'customers_authorization' => ($acc_type=='b'?(int)CUSTOMERS_APPROVAL_AUTHORIZATION:0)
    );
    ...
    //Kands BOE add data to customers_extra table
    $sql_data_array = array('customers_id' => $_SESSION['customer_id'],
                            'customers_vat_number' => $vat_num,
                            'customers_trading_type' => $trading_type,
                            'customers_trade_type' => $trade_type,
                            'customers_account_type' => $acc_type,
                            'customers_verified' => 0,
                            'customers_mobile' => $mobile);

    zen_db_perform("customers_extra", $sql_data_array);
    //Kands EOE
...
// END newsletter_subscribe mod 1/1
    ...
    //BOE KandS
    //$_SESSION['customers_verified'] = 0;    //Set this to zero as account wont be verified at this stage  UPDATE - - this is not used as we use zens built in authorisation
    $_SESSION['customers_trade_type'] = $trade_type;    //0, 1, 2, 99 etc
    $_SESSION['customer_account_type'] = $acc_type; //h or b
    //EOE KandS
    ...
    //KandS - changes to select body text according to business or personal
    if($acc_type=='b'){
        $email_text .= "\n\n" . EMAIL_TEXT_BUSINESS . EMAIL_CONTACT . EMAIL_GV_CLOSURE;
