...
while (!$xsell_query->EOF) {
  ...
  $xsell_query->fields['products_image'] = get_image_xref($xsell_query->fields['products_image']);
...
'text' => '<div class="listingProductImage" style="margin-top: 5px; margin-bottom: 5px;"><a href="' . zen_href_link(zen_get_info_page($xsell_query->fields['products_id']), 'products_id=' . (int)$xsell_query->fields['products_id']) . '">' . zen_image(DIR_WS_IMAGES . $xsell_query->fields['products_image'], $xsell_query->fields['products_name'], SMALL_IMAGE_WIDTH, SMALL_IMAGE_HEIGHT) . '</a></div>'.
  ...
  //(constant(XSELL.$mxsell._DISPLAY_NAME)=='true'? '<div class="familyViewItem"><h4 class="itemTitle"><a href="' . zen_href_link(zen_get_info_page($xsell_query->fields['products_id']), 'products_id=' . (int)$xsell_query->fields['products_id']) . '">' . $xsell_query->fields['products_name'] . '</a></h4></div>':'').

  '<div class="familyViewItem"><h4 class="itemTitle"><a href="' . zen_href_link(zen_get_info_page($xsell_query->fields['products_id']), 'products_id=' . (int)$xsell_query->fields['products_id']) . '">' . $xsell_query->fields['products_name'] . '</a></h4>
  <a href="' . zen_href_link(zen_get_info_page($xsell_query->fields['products_id']), 'products_id=' . (int)$xsell_query->fields['products_id']) . '">' . zen_products_lookup($xsell_query->fields['products_id'], 'products_model') . '</a>
  </div>'.
