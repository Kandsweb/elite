...
//
 ...
 if (!isset($_SESSION['sign_up_pass'])){
    if(isset($_GET['a']) && $_GET['a']=='E5D1'){
      $resend=true;
      $_POST['email']= $_GET['e'];
    }else{
      zen_redirect(zen_href_link(FILENAME_SUBSCRIBE_CHECK, 'e=' .$_POST['email'] . '&f=' . $_POST['email_format']));
    }
  }else{
  $_POST['email'] = $_SESSION['sign_up_pass']['email'];
  $_POST['email_format'] = $_SESSION['sign_up_pass']['format'];

  }

  unset($_SESSION['sign_up_pass']);
/////////////////////////////////////////////////////
...
// include template specific file name defines
  ...
  $subscribe = false;
    ...
    if(!$resend){
      if ($check_cust_email->fields['total'] > 0) {
        $error = true;
        $messageStack->add_session('subscribe', SUBSCRIBE_DUPLICATE_CUSTOMERS_ERROR);
      } elseif ($check_news_email->fields['total'] > 0) {
        echo $check_news_email_status;
        $error = true;
        $messageStack->add_session('subscribe', SUBSCRIBE_DUPLICATE_NEWSONLY_ERROR);
      } else {
        $subscribe = true;
      }
    }
    if($subscribe || $resend){
        ...
        if(!$resend){
          ...
          $db->Execute("update " . TABLE_SUBSCRIBERS .
            " set confirmed = '".$confirm_code."' WHERE email_address = '" . zen_db_input($email_address)."' AND (confirmed != '1' AND confirmed IS NOT NULL) LIMIT 1"
          );
        }
  ...
  $email_text = 'A Newsletter-Only Subscriber using the address ' . $email_address . "\n" .
    'was added on ' . strftime(DATE_FORMAT_LONG) . '.';
            ...
            $email_text = 'A Newsletter-Only Subscriber using the address ' . $email_address . "\n" .
                          'requested a new confirmation email on ' . strftime(DATE_FORMAT_LONG) . '.';
          }
