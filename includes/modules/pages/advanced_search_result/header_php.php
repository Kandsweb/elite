...
//Clear any filters that may be set
$_SESSION['OptionFilter']->clear_filters();
...
///KandS Check for code search and ensure a space is the 4th char
for($i=0; $i<strlen($_GET['keyword']);$i++){
  $letters_list[$i] = substr($_GET['keyword'],$i,1);
}
//$letters_list = str_split($_GET['keyword']);  //Can't use this as 1&1 is using to old of version of php
if( (!is_numeric($letters_list[0])) && (is_numeric($letters_list[1])) && (is_numeric($letters_list[2])) ){
  //
  if($letters_list[3]!=" "){
    $_GET['keyword'] = substr($_GET['keyword'],0,3) . " " . substr($_GET['keyword'],3);
  }
}
//////////////////////////////////////////////////////////////////
...
$select_str = "SELECT DISTINCT " . $select_column_list .
              " m.manufacturers_id, p.products_model, p.products_id, pd.products_name, p.products_price, p.products_tax_class_id, p.products_price_sorter, p.products_qty_box_status, p.master_categories_id ";
...
//echo __FILE__ . '/' . __LINE__ . ' - ' . $select_str;
// Notifier Point
