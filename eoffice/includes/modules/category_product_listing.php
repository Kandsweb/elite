...
<?php if ($action == '') { ?>
        ...
        <?php echo '<a href="' . zen_href_link(FILENAME_IMAGE_ADDER, 'products_filter=' . $products->fields['products_id'] . '&current_category_id=' . $current_category_id . '&action=layout_new') . '">' . zen_image(DIR_WS_IMAGES . 'icon_image_handler.gif', ICON_IMAGE_ADDER) . '</a>'; ?>
        <?php echo '<a href="' . zen_href_link(FILENAME_EASY_XSELL, 'products_filter=' . $products->fields['products_id'] . '&current_category_id=' . $current_category_id) . '">' . zen_image(DIR_WS_IMAGES . 'icon_cross_sell.gif', ICON_EASY_CROSS_SELL) . '</a>';
...
<?php
	//Kands
        if (zen_has_product_attributes($products->fields['products_id'], 'false')) {
          echo '<a href="' . zen_href_link(FILENAME_QUICK_ATTRIBUTES, 'cPath=' . $cPath . '&pID='  . $products->fields['products_id']) . '&attribute_page=1' . '">' . ((!empty($products->fields['products_id']) && zen_has_product_attributes($products->fields['products_id'], 'false')) ? zen_image(DIR_WS_IMAGES . 'icon_attributes_on.gif', ICON_ATTRIBUTES) : zen_image(DIR_WS_IMAGES . 'icon_attributes.gif', ICON_ATTRIBUTES)) . '</a>';
        } else {
					 echo '<a href="' . zen_href_link(FILENAME_QUICK_ATTRIBUTES, '&pID=' . $products->fields['products_id'] . '&cPath=' . $current_category_id) . '&attribute_page=1' . '">' . zen_image(DIR_WS_IMAGES . 'icon_attributes.gif', ICON_ATTRIBUTES) . '</a>';
