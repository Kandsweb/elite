 ...
 */
  ...
  define('HEADING_TITLE', IFA_HEADING_TITLE);
  ...
  if ($action == 'set_products_filter') {
    ...
    zen_redirect(zen_href_link(FILENAME_IMAGE_ADDER, 'page=manager&products_filter=' . $_GET['products_filter']));
  }
  ...
  if($flag_save && $action = 'layout_info')$action='layout_new';
...
$curr_page = FILENAME_IMAGE_ADDER;
  ...
  for ($i=0; $i < $count; $i++) {
    ...
    if ( $_GET['imgName'] == $tmp_image_name ) {
      ...
      echo '<tr id="defaultSelected" class="dataTableRowSelected" onmouseover="rowOverEffect(this)" onmouseout="rowOutEffect(this)" onclick="document.location.href=\''
        . zen_href_link(FILENAME_IMAGE_ADDER, 'products_filter=' . $_GET['products_filter']
...
?>
      ...
      <td class="dataTableContent" align="center" valign="top">
        <?php
          ...
          if (is_file($image_file_medium_full)) {
            echo ' <a href="' . zen_href_link(FILENAME_IMAGE_ADDER, 'imgName='
      ...
      <td class="dataTableContent" align="right"><?php
        if ( $_GET['imgName'] == $tmp_image_name ) {
          ...
          echo '<a href="' . zen_href_link(FILENAME_IMAGE_ADDER, 'products_filter=' . $_GET['products_filter']
            . '&imgName=' . $tmp_image_name . '&action=layout_info')
...
//      $height = min($height, intval(MEDIUM_IMAGE_HEIGHT));
      ...
      $contents = array('align' => 'center', 'form' => zen_draw_form('image_define', FILENAME_IMAGE_ADDER,
      ...
      $contents[] = array('align' => 'center', 'text' => '<br />' .
        ' <a href="' . zen_href_link(FILENAME_IMAGE_ADDER, 'imgName='
        ...
        . zen_image_button('button_delete.gif', IMAGE_DELETE) . '</a> &nbsp;'
        .' <a href="' . zen_href_link(FILENAME_IMAGE_ADDER,
    ...
    case 'layout_new':
      ...
      $contents = array('form' => zen_draw_form('image_define', FILENAME_IMAGE_ADDER,
      ...
      if ($no_images == 1) {
        ...
        $dir = @dir(DIR_FS_CATALOG_IMAGES);
            ...
            $contents[] = array('text' => zen_draw_hidden_field('imgSuffix', $selected_image_suffix) );
      }

      // Image fields
      $contents[] = array('text' => '<br /><strong>' . TEXT_INFO_DEFAULT_IMAGE . '</strong><br />'
          . TEXT_INFO_DEFAULT_IMAGE_ADDER_HELP . '<br />'
...
//                . IH_NEW_NAME_COPY_IMAGES . '<br />'
      ...
      $contents[] = array('align' => 'center', 'text' => '<br />' . '<a href="' . zen_href_link(FILENAME_CATEGORIES, 'cPath=' . $current_category_id) . '&pID=' . $products_filter . '">' .   zen_image_button('button_return.jpg', 'Return to Catalogue') . '</a>');
      break;
    case 'layout_delete':
