 ...
 $action = isset($_GET['action'])?$_GET['action']:'';
  ...
  switch ($action){
    case 'cc':  //Change Category
     ...
     $products_query_raw = "SELECT DISTINCT  p.products_id, p.products_model, pd.products_name, pd.products_viewed, pva.viewed_offset
                           FROM
    ...
    case 'set':
      if(isset($_GET['pid'])&&isset($_GET['nv'])){
          ...
          //now do offset
            //$org_offset = get_viewed_offset($_GET['pid']);
      ...
      zen_redirect(zen_href_link('adjust_popularity.php', 'action=cc&new_category_id=' . $_GET['c']));
      break;
...
<table border="0" width="100%" cellspacing="2" cellpadding="2"><?php //Whole pg table ?>
  ...
  <tr class="dataTableHeadingRow">
    ...
    <th class="dataTableHeadingContent" align="left" width="60%">Name</th>
  ...
  <?php while(!$products->EOF){
    ...
    if($last_basecode != $base_code){
      if($last_basecode != ''){
        echo '<tr><td colspan="10">' . zen_draw_separator('pixel_black.gif','100%', '1') . '</tr>';
        $flag = true;
      }else $flag=true;
    }else $flag = false;
    $last_basecode = $base_code;
    echo '<tr class="dataTableRow" onmouseover="rowOverEffect(this)" onmouseout="rowOutEffect(this)">';
    echo '<td class="dataTableContent" align="center">' . $products->fields['products_id'] . '</td>';
    ...
    echo '<td class="dataTableContent" align="center">' . $products->fields['products_model'] . '</td>';
    echo '<td class="dataTableContent">' . $products->fields['products_name'] . '</td>';
    echo '<td class="dataTableContent" align="center">' . ($products->fields['products_viewed']- $products->fields['viewed_offset']) . '</td>';
    echo '<td class="dataTableContent" align="center">' . $products->fields['viewed_offset'] . '</td>';
    echo '<td class="dataTableContent" align="center">' . zen_draw_input_field('new_value_'.$products->fields['products_id'], $products->fields['products_viewed'],'size=5 onkeydown="if (event.keyCode == 13)vUpdate(\'' . $products->fields['products_id'] . '\',\'' . $products->fields['products_viewed'] . '\')" id="newValue_'.$products->fields['products_id'].'"') .zen_draw_hidden_field('old_value_'.$products->fields['products_id'], $products->fields['products_viewed'])  . '</td>';
            echo '</td>';
    //---------------
    echo '<td class="dataTableContent" align="left">';
    echo '&nbsp;&nbsp;' . zen_image(DIR_WS_IMAGES . 'icon_save.gif', 'Update','','','onclick="javascript:vUpdate(\'' . $products->fields['products_id'] . '\',\'' . $products->fields['products_viewed'] . '\')"');
    echo '&nbsp;&nbsp;&nbsp';
    //echo '</td>';
    //--------------
    //echo '<td class="dataTableContent" align="center" width="6%">';
      if($products->fields['viewed_offset'] !=NULL &&  $products->fields['viewed_offset'] > 0){
        echo '<a href="' . zen_href_link('adjust_popularity.php', 'action=reset&pid='.$products->fields['products_id']). '&c=' . $cCat . '">' . zen_image(DIR_WS_IMAGES . 'icon_reset.gif', 'Reset') . '</a>';
      }else{
         echo '&nbsp;&nbsp;&nbsp;';
      }
      echo '&nbsp;&nbsp;&nbsp;';
    ...
    //---------------
    //echo '<td class="dataTableContent" align="center" width="6%">';
      if($products->fields['viewed_offset'] !=NULL &&  $products->fields['products_viewed']!=$products->fields['viewed_offset']){
        echo '<a href="' . zen_href_link('adjust_popularity.php', 'action=remove&pid='.$products->fields['products_id']). '&c=' . $cCat . '">' . zen_image(DIR_WS_IMAGES . 'icon_delete.gif', 'Remove') . '</a>';
      }
    echo '&nbsp;&nbsp;&nbsp;&nbsp;';
    echo '</td>';
    //--------------
