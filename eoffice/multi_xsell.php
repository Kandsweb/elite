...
<?php if(!defined('MXSELL_ENABLED') ) { ?>
  <a href="<?php echo zen_href_link(FILENAME_MXSELL_PRODUCTS,'action=install'); ?>"><?php echo zen_image_button('button_install.gif', TEXT_INSTALL); ?></a>
<?php } elseif($action != 'remove') { ?>
  <a href="<?php echo zen_href_link(FILENAME_MXSELL_PRODUCTS,'action=remove'); ?>"><?php echo zen_image_button('button_remove.gif', TEXT_REMOVE); ?></a>
<?php } ?>
      </td>
    </tr>
  </table>
...
<!-- <form method='get' action='FILENAME_MXSELL_PRODUCTS'> -->
  ...
  <?php for ( $counter = 1; $counter <= MXSELL_NUM_OF_TABLES; $counter++ ) {
    ...
    <div style="padding:20px;padding-top:0px; float:left; width:40%;">
		<h3 style="color:#0066FF;">Clean up cross-sell <?php echo $mxs_table; ?></h3>
...
<?php
		 if(!isset($xsell_array)){
		  echo '<a href="'.zen_href_link(FILENAME_MXSELL_PRODUCTS, 'action=list_all').'"><h3 style="color:#0066FF;">List All Cross-Sells ' . $mxs_table . '</h3></a>'; 
		 }
		 else{
